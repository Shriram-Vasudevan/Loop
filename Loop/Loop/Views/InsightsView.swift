...
struct InsightsView: View {
        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 24) {
                ...
                responseSummary(analysis)
                    .opacity(animateIn ? 1 : 0)
                    .offset(y: animateIn ? 0 : 20)
                if !analysis.allKeywords.isEmpty {
                    keywordsView(analysis)
                        .opacity(animateIn ? 1 : 0)
                        .offset(y: animateIn ? 0 : 20)
                }
                if !analysis.allNames.isEmpty {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Analyzed from your responses")
    ...
    private func statsOverview(_ analysis: DailyAnalysis) -> some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
            InsightCard(title: "Speaking Pace", value: String(format: "%.0f", analysis.averageSpeakingPace), unit: "WPM", icon: "waveform", color: accentColor)
            InsightCard(title: "Total Words", value: "\(analysis.totalWordCount)", unit: "words", icon: "text.bubble", color: accentColor)
            InsightCard(title: "Self References", value: String(format: "%.0f", analysis.averageSelfReferencePercentage), unit: "%", icon: "person", color: accentColor)
            InsightCard(title: "Duration", value: String(format: "%.0f", analysis.averageDuration), unit: "sec", icon: "clock", color: accentColor)
        }
    }
    ...
    private func responseSummary(_ analysis: DailyAnalysis) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("Response Patterns")
                ...
                .foregroundColor(textColor)
            VStack(spacing: 12) {
                PatternRow(icon: "arrow.left.arrow.right", title: "Time Focus", detail: getTensePattern(analysis), color: accentColor)
                Divider()
                PatternRow(icon: "hand.point.up", title: "Self Expression", detail: getSelfExpressionPattern(analysis), color: accentColor)
            }
            .padding(16)
            .background(
    ...
    private func keywordsView(_ analysis: DailyAnalysis) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            FlowLayout(spacing: 8) {
                ForEach(analysis.allKeywords, id: \.self) { keyword in
                    Text(keyword)
    ...
    private func connectionsView(_ analysis: DailyAnalysis) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 12) {
                    ForEach(analysis.allNames, id: \.self) { name in
    ...
    private func processingView(_ analysis: DailyAnalysis) -> some View {
        VStack(spacing: 24) {
            ProgressCircle(progress: Double(analysis.promptAnalyses.count) / 3.0, color: accentColor)
                .frame(width: 80, height: 80)
            Text("Analyzing Your Responses")
                .font(.system(size: 24, weight: .light))
                .foregroundColor(textColor)
            Text("\(analysis.promptAnalyses.count) of 3 loops processed")
    ...
    private var noAnalysisView: some View {
        VStack(spacing: 16) {
            ...
            Text("No Insights Yet")
                .font(.system(size: 24, weight: .light))
                .foregroundColor(textColor)
            Text("Record your responses to see insights")
                .font(.system(size: 16, weight: .regular))
                .foregroundColor(textColor.opacity(0.7))
        }
    }
    ...
    private func getTensePattern(_ analysis: DailyAnalysis) -> String {
        if analysis.averagePastTensePercentage > analysis.averageFutureTensePercentage {
    ...
    private func getSelfExpressionPattern(_ analysis: DailyAnalysis) -> String {
        let percentage = analysis.averageSelfReferencePercentage
        if percentage > 70 {
