...
struct RecordFreeResponseView: View {
    ...
    private var mainRecordingView: some View {
        VStack(spacing: 40) {
            ...
            if isRecording {
                VStack(spacing: 16) {
                    // Only show the timer for non-premium users
                    if shouldShowTimer {
                        HStack(spacing: 12) {
                            PulsingDot()
                        ...
                        // For premium users, just show recording indicator without timer
                        HStack(spacing: 12) {
                            PulsingDot()
                            Text("Recording")
                                .font(.system(size: 20, weight: .regular))
    ...
    // Helper functions remain largely the same but with premium modifications
    ...
    private func startRecordingWithTimer() {
        ...
        // Set time based on premium status
        timeRemaining = premiumManager.getMaxRecordingDuration()
        startTimer()
    }
    ...
    private func retryRecording() {
        if retryAttempts > 0 {
            ...
            isPostRecording = false
            isRecording = false
            // Reset time based on premium status
            timeRemaining = premiumManager.getMaxRecordingDuration()
        }
    }
