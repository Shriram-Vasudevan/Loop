...
struct RecordFeaturedLoopView: View {
        ...
        if let audioFileURL = audioManager.getRecordedAudioFile() {
            Task {
                let loop = await loopManager.addLoop(
                    ...
                    isVideo: false,
                    prompt: prompt,
                    isDailyLoop: true, isFollowUp: false, isSuccess: false, isUnguided: false, isDream: false, isMorningJournal: false
                )
