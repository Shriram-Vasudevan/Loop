...
struct RecordFollowUpLoopView: View {
        ...
        if let audioFileURL = audioManager.getRecordedAudioFile() {
            Task {
                let loop = await loopManager.addLoop(
                    ...
                    isVideo: false,
                    prompt: prompt,
                    isDailyLoop: false, isFollowUp: true, isSuccess: false, isUnguided: false, isDream: false, isMorningJournal: false
                )
